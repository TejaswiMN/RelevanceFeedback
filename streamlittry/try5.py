...
class ArxivRetriever:
    ...
    def parse_arxiv_response(response_text: str) -> List[Paper]:
        ...
        for entry in root.findall("atom:entry", ns):
            ...
            papers.append(Paper(title=title, authors=authors, summary=summary, url=url))
...
class RocchioFeedback:
    ...
    def get_modified_query(
        ...
        rel_vectors = (
            tfidf_matrix[1 : len(relevant_docs) + 1] if relevant_docs else None
        )
        non_rel_vectors = (
            tfidf_matrix[len(relevant_docs) + 1 :] if non_relevant_docs else None
        )
...
class RetrievalSystem:
    ...
    def rank_papers(self, papers: List[Paper], query: str) -> List[Tuple[Paper, float]]:
        ...
        processed_texts = [
            self.text_processor.preprocess_text(paper.full_text) for paper in papers
        ]
    ...
    def get_initial_results(
        self, query: str, relevant_papers: List[Paper] = None
    ...
    def get_expanded_query(
        self, original_query: str, relevant_papers: List[Paper]
    ) -> str:
        ...
        processed_docs = [
            self.text_processor.preprocess_text(doc) for doc in relevant_docs
        ]
...
def calculate_dcg(relevance_scores, k=None):
    if k is None or k > len(relevance_scores):
        k = len(relevance_scores)
    return sum(relevance_scores[i] / np.log2(i + 2) for i in range(k))
...
def main():
    ...
    if "relevant_counts" not in st.session_state:
        st.session_state.relevant_counts = []  # Track relevant counts per iteration
    if "dcg_scores" not in st.session_state:
        st.session_state.dcg_scores = []  # Track DCG scores
    if "prev_relevant_count" not in st.session_state:
        st.session_state.prev_relevant_count = 0  # Track the previous count
    ...
    if query:
        ...
        st.markdown(
            f"**Previously marked relevant papers: {len(st.session_state.relevant_papers)}**"
        )

    # Reset everything if query changes
    if query and (
        not hasattr(st.session_state, "current_query")
        or query != st.session_state.current_query
    ):
        ...
        initial_results = st.session_state.retrieval_system.get_initial_results(
            query, st.session_state.relevant_papers
        )
    ...
    if query:
        ...
        if st.button("📝 Submit Feedback and Get New Results", type="primary"):
            # to update metrics
            current_relevant_count = len(st.session_state.relevant_papers) + len(
                st.session_state.temp_relevant_indices
            )
            ...
            increased_effectiveness = calculate_increased_effectiveness(
                current_relevant_count, st.session_state.prev_relevant_count
            )
            print("increased effectiveness: ", increased_effectiveness)
            st.session_state.prev_relevant_count = current_relevant_count

            relevance_scores = [
                1 if i in st.session_state.temp_relevant_indices else 0
                for i in range(len(st.session_state.current_papers))
            ]
            print("relevance scores: ", relevance_scores)
            dcg_score = calculate_dcg(relevance_scores)
            st.session_state.dcg_scores.append(dcg_score)
            print("DCG: ", dcg_score)

            # Store currently marked papers as relevant
            newly_relevant = [
                st.session_state.current_papers[i]
            ...
            st.session_state.relevant_papers = [
                x
                for x in st.session_state.relevant_papers
            ...
            new_results = st.session_state.retrieval_system.get_initial_results(
                expanded_query, st.session_state.relevant_papers
            )
            ...
            # Plot the graph
            st.markdown("### Relevance Feedback Progress")
            df = pd.DataFrame(
                {
                    "Iteration": list(
                        range(1, len(st.session_state.relevant_counts) + 1)
                    ),
                    "Relevant Docs": st.session_state.relevant_counts,
                }
            )
            fig, ax = plt.subplots()
            ax.plot(df["Iteration"], df["Relevant Docs"], marker="o", linestyle="-")
            ax.set_xlabel("Iteration")
            ax.set_ylabel("Number of Relevant Docs")
            ax.set_title("Relevance Feedback Progress")
            # Save the graph
            fig.savefig("relevance_feedback_progress.png")
