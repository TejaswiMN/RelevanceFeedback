...
def calculate_dcg(relevance_scores, k=None):
    ...
    return sum(
        relevance_scores[i] / np.log2(i + 2) for i in range(k)
    )
...
def main():
    ...
    if query:
        ...
        if st.button("📝 Submit Feedback and Get New Results", type="primary"):
            # to update metrics
            current_relevant_count = len(st.session_state.relevant_papers) + len(st.session_state.temp_relevant_indices)
            ...
            df = pd.DataFrame({
                "Iteration": list(range(1, len(st.session_state.relevant_counts) + 1)),
